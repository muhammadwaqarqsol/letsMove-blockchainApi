...
export default function Content() {
  ...
  useEffect(() => {
    const setOpenQuery = router?.pathname?.split('/');
    let joinedQuery;

    if (setOpenQuery && setOpenQuery.length > 3) {
      // Remove the last element
      setOpenQuery.pop();
      joinedQuery = setOpenQuery.join('/');
    } else {
      // Perform normal join
      joinedQuery = setOpenQuery?.join('/');
    }
    setOpen(joinedQuery);
    setChildActive(router?.pathname);
  }, [router?.pathname]);
            ...
            <Collapsible.Trigger asChild>
              <div
                className={`
                     ${
                       active === item.key ||
                       router?.pathname.endsWith(item.key)
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent side="right">
                        <p className="text-base z-[9999]">{item.title}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                )}
                  ...
                  <Link
                    ...
                    className={`
                        ${
                          ...
                          router?.pathname.endsWith(childItem.key)
                            ? 'text-primary bg-secondary/80 font-semibold '
                            : ''
                        }
...
const SIDEBAR_DATA: any = [
  ...
  //     },
  //     {
  //       key: '/admin/reports/project',

  //       icon: <i className=" fa-solid fa-diagram-project p-4 rounded-full" />,
  //       title: 'Projects',
  //     },
  //     {
  //       key: '/admin/reports/tickets',
